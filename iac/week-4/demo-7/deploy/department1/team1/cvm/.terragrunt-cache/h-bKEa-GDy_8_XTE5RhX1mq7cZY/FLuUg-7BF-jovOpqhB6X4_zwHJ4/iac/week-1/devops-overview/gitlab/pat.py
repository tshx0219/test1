...
endpoint = "https://gitlab.devopscamp.com/"
...
login = "root"
password = "s2KeNZHjgHaVFLQvrWxaCbdFQEoOv2t0ubO9kEZB4U9xyLZxGG290Y1Zk1SSCQvv"
...
def obtain_authenticity_token(cookies):
    ...
    token = soup.find('meta', attrs={'name': 'csrf-param', 'content': 'authenticity_token'}).get('value')
    return token
...
def obtain_personal_access_token(name, expires_at, csrf, cookies, authenticity_token):
    ...
    soup = BeautifulSoup(r.text, "lxml")
    print(soup)
    token = soup.find('input', id='new_token').get('value')
    return token
